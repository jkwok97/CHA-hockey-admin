...
export class AddChampComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.awardType.valueChanges
      .pipe(takeWhile(() => this._alive))
      .subscribe((id) => {
        this.showPlayerField = this.allowPlayerField(id);
        this.showPlayerField
          ? this.wonByPlayer.setValidators([Validators.required])
  ...
  getAwardWinner(id: number) {
    this._championsService
      .getAwardWinnerById(id)
      .pipe(takeWhile(() => this._alive))
      .subscribe((winner) => {
        this.winner = winner;
  ...
  getPlayers(name: string) {
    ...
    this._teamsService
      .getPlayersByTeamName(name, this.currentSeason, "Regular")
      .pipe(take(1))
      .subscribe((players) => {
  ...
  onEditSave(id) {
    ...
    this._championsService
      .updateAwardWinner(id, awardWinner)
      .pipe(takeWhile(() => this._alive))
      .subscribe(
        (resp) => {
          this._displayService.popupTrigger(resp["message"]);
